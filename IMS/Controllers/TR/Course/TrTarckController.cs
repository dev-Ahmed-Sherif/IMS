...
namespace IMS.Controllers.TR.Course
{
    ...
    [ApiController]
    public class TrTarckController : ControllerBase

    {
        private TrTrackService _Service;


        public TrTarckController(TrTrackService AddTrack)
        {
            _Service = AddTrack;
        }
        [HttpPost("Add")]
        public IActionResult Add([FromBody] TrTrackVM add)
        {
            var _response = _Service.Add(add);
            return new JsonResult(_response);
        }
        [HttpPut("update")]
        public IActionResult Update([FromBody] TrTrackVM update)
        {
