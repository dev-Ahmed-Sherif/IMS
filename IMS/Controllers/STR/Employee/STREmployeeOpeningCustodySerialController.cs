...
namespace IMS.Controllers.STR.Employee
{
    ...
    [ApiController]
    public class STREmployeeOpeningCustodySerialController : ControllerBase
    {
        private StrEmployeeOpeningCustodySerialService _Row;

        public STREmployeeOpeningCustodySerialController(StrEmployeeOpeningCustodySerialService ID)
        {
            _Row = ID;
        }

        [HttpPost("Add")]
        public IActionResult Add([FromBody] StrEmployeeOpeningCustodySerialVM ID)
        {
            var _response = _Row.Add(ID);
            return new JsonResult(_response);
        }
        [HttpPut("update")]
        public IActionResult UpdateItem([FromBody] StrEmployeeOpeningCustodySerialVM ID)
        {
